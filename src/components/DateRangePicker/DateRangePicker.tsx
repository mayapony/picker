...
const DateRangePicker: React.FC<DateRangePickerProps> = ({
  ...
  const formatRange = (
    ...
    switch (rangeType) {
      ...
      case "quarter":
        return [
          `${start.year()}-Q${start.quarter()}`,
          `${end.year()}-Q${end.quarter()}`,
        ];
      case "year":
  ...
  const handleDateSelect = (date: dayjs.Dayjs) => {
    ...
    if (!startDate || (startDate && endDate)) {
      ...
      // 完成选择
      const rangeEnd =
        rangeType === "week" ? localDate.endOf("week") : localDate.endOf("day");
    ...
    while (
      ...
      if (startDate) {
        if (rangeType === "week") {
          ...
          if (endDate) {
            // 有结束日期时，判断是否在范围内
            const actualStartDate = startDate.isBefore(endDate)
              ? startDate
              : endDate;
            const actualEndDate = startDate.isBefore(endDate)
              ? endDate
              : startDate;

            // 对于周选择，我们需要比较周的起始和结束日期
            const actualStartWeekStart = actualStartDate.startOf("week");
            const actualEndWeekEnd = actualEndDate.endOf("week");

            isSelected =
              weekStart.isSame(actualStartWeekStart, "day") ||
              weekEnd.isSame(actualEndWeekEnd, "day") ||
              (weekStart.isAfter(actualStartWeekStart, "day") &&
                weekEnd.isBefore(actualEndWeekEnd, "day"));
            isStart = weekStart.isSame(actualStartWeekStart, "day");
            isEnd = weekEnd.isSame(actualEndWeekEnd, "day");
          } else {
            // 只有开始日期时，高亮当前选中的周
        ...
        {months.map((month) => {
          ...
          return (
            <button
              ...
              className={`date-range-picker__month-btn ${
                isSelected ? "selected" : ""
      ...
      <div className="date-range-picker__quarter-grid">
        {quarters.map((q) => {
          ...
          const isSelected =
            startDate &&
            endDate &&
            (qStart.isAfter(startDate, "day") ||
              qStart.isSame(startDate, "day")) &&
          ...
          return (
            <button
              ...
              className={`date-range-picker__quarter-btn ${
                isSelected ? "selected" : ""
              } ${isStart ? "start" : ""} ${isEnd ? "end" : ""}`}
  ...
  return (
    <div className={`date-range-picker ${className}`}>
      <div className="date-range-picker__header">
        ...
        <div className="date-range-picker__title">{currentDate.year()}年</div>
        <button
          className="date-range-picker__nav-btn"
          ...
          <div className="date-range-picker__months-grid">
            {Array.from({ length: 12 }, (_, monthIndex) => {
              ...
              return (
                <div
                  key={monthIndex}
                  className="date-range-picker__month-section"
                >
      ...
      <div className="date-range-picker__actions">
        <button
          className="date-range-picker__action-btn date-range-picker__action-btn--reset"
          onClick={handleReset}
        >
          重置
        </button>
